...
def get_all_tweets(screen_name):
    ...
    outtweets = [
        [tweet.created_at, tweet.text.encode("utf-8"), "http://twitter.com/" + screen_name + "/status/" + tweet.id_str]
        for tweet in alltweets]

    f = open(r"C:\Users\alija\Downloads\text_tags.txt", "r")

    for tweet in alltweets:
        for line in f:
            if line.strip() in tweet.text.lower():
                dictionaries.append({'img': "", 'text': tweet.text,
                                     'link': "http://twitter.com/" + screen_name + "/status/" + tweet.id_str,
                                     'msg': "This was flagged because it contained " + line, 'date': tweet.created_at})
        ...
        for status in tweets:
            ...
            if (len(media) > 0 and downloaded < num_tweets):
                parse_image(media[0]['media_url'],
                            ...
                            status.entities.get(0, 'created_at'))
            downloaded += 1
